...
LYRICS = u"2:1:Lyssna;inte;på;mig\n"\
...
def get_text_object(idx: int, text: str, phase: int, anim_start: int, anim_length: int, position: tuple=(0.0, 0.0, 0.0)):
    ...
    obj.location = (0.0 + position[0], -(LINES_SPACE * (idx % 3)) + position[1], 0.0 + position[2])
...
def drop_lyrics(lyrics):
    for idx, lyr in enumerate(LYRICS_PARSED):
        ...
        for idx2, t in enumerate(lyr.text):
            o, d_dimension = get_text_object(idx, t, lyr.phase, lyr.anim_start * FPS, lyr.anim_length, (spacing_accumlator + idx2 * SPACE_SIZE, 0, 0))
